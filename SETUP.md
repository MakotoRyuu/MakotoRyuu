...
Python 按北京时间日期从列表中顺序循环选择一句（以 2026-09-20 为第一句的起点），同一天重复运行不切换；修改列表顺序或数量后当天选句可能改变。每日一言输出为 `quotes/daily-quote.svg`，与 `holiday/holiday-calendar.svg` 是两张独立静态图片，放在节假日日历上方。无动画。字体与日历使用相同来源，长句自动换行并增加图片高度。
